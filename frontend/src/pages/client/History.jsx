...
export default function History() {
    ...
    const fetchCourses = async (page = 1) => {
        ...
        try {
            const response = await axios.get(
                `http://127.0.0.1:8000/api/courses/${user.client.id}?page=${page}`,
                {
                    headers: {
            ...
            console.log("Courses récupérées :", response.data.courses);
            setPagination({
    ...
    const filteredTrips = courses.filter((t) => {
        const matchFilter = filter === "toutes" ? true : t.status === filter;
        const matchSearch =
            (t.adresse_depart + t.destination).toLowerCase().includes(search.toLowerCase());
                ...
                <main className="flex-1 p-6 space-y-6">
                    ...
                    <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm mb-6">
                        ...
                        <div className="flex flex-wrap gap-2">
                            ...
                            {["toutes", "terminee", "confirmee", "en attente", "annuler"].map((f) => (
                                <button
                                    key={f}
                                    onClick={() => setFilter(f)}
                                ...
                                {filteredTrips.map((trip) => (
                                    ...
                                    >
                                        ...
                                        <div className="pt-3 border-t text-xs text-slate-500">
                                            {trip.chauffeur
                                                ? <>Chauffeur <span className="font-semibold text-slate-700">{trip.chauffeur.user.first_name} {trip.chauffeur.user.last_name}</span></>
